...
def _simulate(task, initial_tape, program, answer):
    ...
    expected_output = {
        output_start + offset: read_digit(digit)
        for offset, digit in enumerate(answer)
    }
    head0 = 0
    head1 = 0
    head0_reads = []
    ...
    def output_is_complete():
        if any(tape.get(pos, READ_BLANK) != token for pos, token in expected_output.items()):
            return False
        return all(
            position in expected_output or token == READ_BLANK
            for position, token in tape.items()
...
def generate_mul(a, b):
    ...
    # Copy MAIN cells only, skipping each CARRY and WIP block between digits.
    for index, digit in enumerate(answer):
        program.step(
            head0_move=MOVE_RIGHT,
            head1_move=MOVE_RIGHT,
...
class ArithmeticTickDataLoader:
    ...
    def _collate(self, batch):
        ...
        fields = {
            ...
            "prev_head0_moves": (MOVE_STAY, []),
            "prev_head1_moves": (MOVE_STAY, []),
            "prev_head0_writes": (WRITE_NOOP, []),
            "prev_head1_writes": (WRITE_NOOP, []),
            "target_head0_moves": (MOVE_STAY, []),
            "target_head1_moves": (MOVE_STAY, []),
            "target_head0_writes": (WRITE_NOOP, []),
            "target_head1_writes": (WRITE_NOOP, []),
        }
        loss_mask = []
