...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        for module in self.modules():  # restore special inits clobbered by _init_weights
    ...
    def forward(self, idx, kv_caches=None):
        """
        idx: (B, T_new) chunk token ids.
        kv_caches: list of per-layer SSM states (g, v_prev), None on the first chunk.
                   Carried across chunks with BPTT (caller detaches between optimizer steps).
        Returns logits (B, T_new, vocab_size) and new per-layer states.
        """
